...
function IconTile({ children }: { children: ReactNode }) {
  return (
    <div className="flex h-[46px] w-[46px] shrink-0 items-center justify-center rounded-[9px] bg-linear-to-b from-[#bf84f5] to-[#7e22ce] shadow-[0_8px_20px_rgba(126,34,206,0.25)]">
      {children}
    </div>
  );
}
...
function ValueItem({
  ...
  return (
    <div className="flex items-start gap-4">
      ...
      <div>
        <h3 className="text-[16px] font-bold leading-tight text-[#111827] sm:text-[17px] md:text-[18px] lg:text-[20px]">{title}</h3>
        <p className="mt-1.5 text-[13px] leading-[1.5] text-[#4b5563] sm:text-[14px] md:text-[15px] lg:max-w-[420px] lg:text-[16px]">{description}</p>
      </div>
    </div>
  );
}
...
export default function AboutUsSection() {
  return (
    <section className="w-full bg-white px-4 py-12 sm:px-6 md:py-16 lg:py-[92px]">
      <div className="mx-auto w-full max-w-[1240px]">
        ...
        <h2 className="mt-7 text-center text-[26px] font-bold leading-[1.18] tracking-[-0.3px] text-[#111827] sm:text-[32px] md:text-[38px] md:tracking-[-0.5px] lg:text-[44px] lg:tracking-[-0.6px] xl:text-[50px]">
          Passionate Designers, Powerful Brands
        </h2>
        <p className="mx-auto mt-5 max-w-[980px] text-center text-[15px] leading-[1.6] text-[#4b5563] sm:text-[16px] md:mt-7 md:text-[17px] lg:text-[18px]">
        ...
        <p className="mx-auto mt-4 max-w-[980px] text-center text-[15px] font-semibold leading-[1.6] text-[#1f2937] sm:text-[16px] md:mt-6 md:text-[17px] lg:text-[18px]">
          From startups to established enterprises, we&apos;ve partnered with diverse clients to create memorable brand
          experiences that stand the test of time.
        </p>

        <div className="mt-8 grid grid-cols-1 gap-x-8 gap-y-8 md:mt-10 md:grid-cols-2 md:gap-x-12 md:gap-y-10 lg:mt-14 lg:gap-x-20">
